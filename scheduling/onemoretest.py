...
ROOMS = [Room(i[0],i[1]) for i in ROOM_LISTING]
...
ALL_COURSES = [DEPT1_COURSES + DEPT2_COURSES + DEPT3_COURSES + DEPT4_COURSES + DEPT5_COURSES + DEPT6_COURSES]


DEPARTMENTS_LIST = [
    [1,'DEPT1', DEPT1_COURSES],
    [2,'DEPT2', DEPT2_COURSES],
    [3,'DEPT3', DEPT3_COURSES],
    [4,'DEPT4', DEPT4_COURSES],
    [5,'DEPT5', DEPT5_COURSES],
    [6,'DEPT6', DEPT6_COURSES],
]

DEPARTMENTS = [
    Department(i[0],i[1], i[2]) for i in DEPARTMENTS_LIST
]
...
class Data:
    def __init__(self):
        self._rooms = [Room(i[0],i[1]) for i in ROOM_LISTING]
        self._times = create_times(DAYS_OF_WEEK, TIME_SLOTS)
        self._courses = ALL_COURSES
        self._depts = [Department(i[0],i[1], i[2]) for i in DEPARTMENTS_LIST]
...
class Schedule:
    def __init__(self, data, department):
        self._data           = data
        self._times          = self._data.get_times()
...
class Population:
    ...
    def shuffle_classes(self):
        for each in self._schedules_list:
            each.shuffle_classes_list()

        
    def mutate_schedule(self, position = 0):
        new_schedule = self._schedules_list[position]
        for i in range(new_schedule.get_classes_num()):
            if i % 2 == 0 :
                new_schedule.overwrite_class(i,self._schedules_list[position].get_classes_list()[i])
        self._schedules_list[position] = new_schedule
...
def create_population(size):
    while True:
        ...
        for i in range(size):
            holder_schedule = Schedule(data, DEPARTMENTS[i])
            try:
